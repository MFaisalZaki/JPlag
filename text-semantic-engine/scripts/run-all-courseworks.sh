...
#                                   interrupted run.
#   ONLY=<pattern>                  only process courseworks whose
#                                   '<module>/<coursework>' path matches this
#                                   shell pattern, e.g. ONLY='SD2005/*'.
#
...
for relative in "${COURSEWORKS[@]}"; do
  ...
  say "    $status — ${total_seconds}s (index ${index_seconds}s + query ${query_seconds}s), peak ${peak_mb} MB, $flagged/$documents document(s) with a match"
done
...
# --------------------------------------------------------------------------
{
  ...
  echo
  echo "OVERALL"
  awk -F, -v wall="$ELAPSED_ALL" -v failures="$FAILURES" 'NR > 1 {
  ...
  END {
    ...
    printf "  with a match     : %d (%.1f%% of documents)\n", flagged, 100 * flagged / docs
